...
parse sample names and join metadata:
...
data.scores %>%
    ...
    separate(col = samples,
             ...
             sep = "_",
             remove = FALSE) %>%
    left_join(metadata, by = "site_name") -> ordination_coordinates
```
...
```{r}
samples <- rownames(parasitic_table_transposed_rarefied)
data.scores <- as.data.frame(vegan::scores(occurrences_t.bray.nmds,
                                           display = "sites"))
data.scores$samples <- rownames(data.scores)
...
```{r}
rm(data.scores, parasitic_table_transposed_rarefied)
```
